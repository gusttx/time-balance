<script lang="ts">
	...
	import { CircleDotDashed, Sparkles } from "@lucide/svelte";
	...
	<ol class="mt-4 flex w-full flex-col gap-2">
		{#each activities.entries as activity (activity.id)}
			<li>
				...
				>
					<CircleDotDashed strokeWidth={2} />
					<div class="flex grow items-center justify-between truncate">
						<div class="truncate">
							{activity.name}
						</div>
						<div
							class={cn("font-dosis text-muted-foreground flex items-center justify-center gap-1", {
	...
	<Drawer.Content>
		...
		<div class="mb-4 flex flex-col gap-4">
			<div class="flex flex-col gap-1">
				...
				<Input id="name" placeholder="Learn Rust" maxlength={30} minlength={1} bind:value={name} bind:ref={input} />
				{#if invalidName}
