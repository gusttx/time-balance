...
pub async fn create_entry(
    ...
    log::debug!(
        "Creating entry with activity '{}' and duration '{}'",
        activity.id,
        duration
    );
...
#[tauri::command]
pub async fn get_entry_page(
    db: State<'_, Db>,
    page: i64,
) -> Result<(Vec<Entry>, i64), CommandError> {
    if page <= 0 {
    ...
    log::debug!(
        "Successfully got entry page {} with {} entries, total entries: {}",
        page,
        result.0.len(),
        result.1
    );
